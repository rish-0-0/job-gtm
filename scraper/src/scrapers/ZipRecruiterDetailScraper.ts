...
export class ZipRecruiterDetailScraper extends JobDetailScraper {
    public async scrapeJobDetails(url: string): Promise<JobDetails> {
        const startTime = Date.now();

        try {
            console.log(`[ZipRecruiterDetailScraper] Scraping: ${url}`);
            ...
            if (!response || response.status() !== 200) {
                ...
                console.log(`[ZipRecruiterDetailScraper] ${error}`);
                return {
                ...
                try {
                    ...
                    if (element) {
                        ...
                        if (text && text.length > 100) {
                            ...
                            console.log(`[ZipRecruiterDetailScraper] Found description with selector: ${selector} (${text.length} chars)`);
                            break;
                        }
                    }
                } catch {
            ...
            console.log(`[ZipRecruiterDetailScraper] ✅ Scraped in ${duration}ms - Description: ${jobDescriptionFull.length} chars, Page: ${fullPageText.length} chars`);
            ...
            console.error(`[ZipRecruiterDetailScraper] ❌ Error: ${errorMsg}`);
            return {
