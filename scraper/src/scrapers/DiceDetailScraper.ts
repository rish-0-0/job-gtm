...
export class DiceDetailScraper extends JobDetailScraper {
    public async scrapeJobDetails(url: string): Promise<JobDetails> {
        const startTime = Date.now();

        try {
            console.log(`[DiceDetailScraper] Scraping: ${url}`);
            ...
            if (!response || response.status() !== 200) {
                ...
                console.log(`[DiceDetailScraper] ${error}`);
                return {
                ...
                try {
                    ...
                    if (element) {
                        ...
                        if (text && text.length > 100) {
                            ...
                            console.log(`[DiceDetailScraper] Found description with selector: ${selector} (${text.length} chars)`);
                            break;
                        }
                    }
                } catch {
            ...
            console.log(`[DiceDetailScraper] ✅ Scraped in ${duration}ms - Description: ${jobDescriptionFull.length} chars, Page: ${fullPageText.length} chars`);
            ...
            console.error(`[DiceDetailScraper] ❌ Error: ${errorMsg}`);
            return {
